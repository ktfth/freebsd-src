...
fn parse_primary(ps: &mut ParseState, now: SystemTime) -> Pred {
    ...
    match tok.as_str() {
        ...
        "-name" => {
            ...
            Pred::Name { pattern: pat, ignore_case: false }
        }
        "-iname" => {
            let pat = ps.require("-iname").to_owned();
            Pred::Name { pattern: pat, ignore_case: true }
        }
        "-path" | "-wholename" => {
            let pat = ps.require("-path").to_owned();
            Pred::Path { pattern: pat, ignore_case: false }
        }
        "-ipath" | "-iwholename" => {
            let pat = ps.require("-ipath").to_owned();
            Pred::Path { pattern: pat, ignore_case: true }
        }
        "-type" => {
...
fn eval(pred: &Pred, entry: &DirEntry, ctx: &mut EvalCtx) -> bool {
    match pred {
        ...
        Pred::Name { pattern, ignore_case } => {
            let name = entry.file_name().to_string_lossy();
            glob_match(pattern, &name, *ignore_case)
        }

        Pred::Path { pattern, ignore_case } => {
        ...
        Pred::Type(ft) => {
            ...
            match ft {
                ...
                // For block/char/pipe/socket we need std::os::unix
        ...
        Pred::Empty => {
            ...
            if meta.is_file() {
                ...
                entry.path().read_dir().map_or(false, |mut d| d.next().is_none())
            } else {
                false
            }
        }
...
fn main() {
    ...
    while pos < raw_args.len() {
        match raw_args[pos].as_str() {
            "-H" => { opts.follow_cmd_links = true; pos += 1; }
            "-L" => { opts.follow_links = true; pos += 1; }
            "-P" => { opts.follow_links = false; pos += 1; }
            "-d" => { opts.depth_first = true; pos += 1; }
            "-x" => { opts.xdev = true; pos += 1; }
            _ => break,
        }
    }
    ...
    let expr: Pred = if expr_args.is_empty() {
        ...
        let mut ps = ParseState { args: &expr_args, pos: 0 };
